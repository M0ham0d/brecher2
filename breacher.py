...
parser.add_argument("-u", help="رابط الهدف", dest='الهدف')
parser.add_argument("--path", help="بادئة مسار مخصص", dest='prefix')
parser.add_argument("--type", help="عيّن النوع ، أي html ، و asp ، و php", dest='type')
parser.add_argument("--fast", help="يستخدم multithreading", dest='fast', action="store_true")
args = parser.parse_args() #arguments to be parsed

الهدف = args.الهدف #Gets tarfet from argument
...
print(DrawText("mohamed"))

print ('''الٱن تمت إعادة برمجة هذه الأداة بواسطتي''')
print ('\033[1;31m--------------------------------------------------------------------------\033[1;m\n')

try:
    target = target.replace('https://', '') #Removes https://
except:
    print ('\033[1;31m[-]\033[1;m -u يمكنك تشغيل الأداة عبر كتابة هذا الأمر')
    print ("python3 breacher.py -h")
    quit()
